...
def scrape_and_build_product_links():
    with sync_playwright() as p:
        ...
        women_footwear = "https://earnkaro.com/product/women-footwear"
        ...
        product_link = [super_seller]  # , home_kitchen, women_footwear, baby_product, men_footwear, men_fashion, women_fashion, grocery, luggage, Accesories, beauty, electronics, personal_care]
        category = ["super_seller", "home_kitchen", "women_footwear", "baby_product", "men_footwear", "men_fashion", "women_fashion", "grocery", "luggage", "Accesories", "beauty", "electronics", "personal_care"]
        ...
        for i in product_link:
            ...
            if count == 0:
                for p in products:
                    ...
                    starting_price_str = attr.get("category_price_starting_from", "").strip()
                    price = float(starting_price_str) if starting_price_str else 0.0
                    product_id = p.get("id", "").strip()
                    percent_dis = float(0)
                    # price_dis = float(price.get("discounted_price", 0))
                    merchant = (attr.get("merchant_name") or "").strip()
                    if merchant:
                        cursor.execute("""insert into products (name,image_url,price,percent,product_id,category) values (%s,%s,%s,%s,%s,%s) """, (name, image_url, price, percent_dis, product_id, category[count]))
            else: 
            # 9️⃣ Add products to dict
                for p in products:
                    ...
                    merchant = (attr.get("merchant_name") or "").strip()
                    if merchant:
                        cursor.execute("""insert into products (name,image_url,price,percent,product_id,category) values (%s,%s,%s,%s,%s,%s) """, (name, image_url, price_dis, percent, product_id, category[count]))
